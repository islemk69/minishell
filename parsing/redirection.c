...
/*   Updated: 2023/05/07 16:44:51 by hel-ouar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**realloc_redir(char **space)
{
	...
	while (space[i])
	{
		if ((is_token_char(space[i][0]) && !space[i][1])
			|| (is_token_char(space[i][0])
				&& is_token_char(space[i][1]) && !space[i][2]))
		{
			size++;
			i++;
		}
...
char	**redir_first(char **realloc)
{
	...
	i = 0;
	j = 0;
	new_tab = ft_gc_malloc(sizeof(char *) * (ft_strlen_dtab(realloc) + 1));
	while (realloc[i])
	{
